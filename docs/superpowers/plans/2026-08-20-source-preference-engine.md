...
- **Ranker formula** (`RANK_WEIGHTS`, all in one object): `pop = log10(downloads+1) / max log10(downloads+1) within the show` (0 when every recording has 0 downloads); `rating = ((avgRating·numReviews) + 4.0·5) / (numReviews + 5) / 5` with a missing rating treated as `numReviews = 0`; `lineage = min(0.30, betty .15 + miller .15 + 16track .10 + lowgen .05)`; `score = 0.45·pop + 0.35·rating + lineage`. Ties: downloads desc, then identifier asc.
